...
angular
		...
		.controller(
				...
				function($scope, $state, $stateParams, ProductService) {
					...
					$scope.productInfo = ""; // 它是一个对象，它封装了所有的理财产品信息
					// 调用函数,商品信息回显
					(function() {
					...
					$scope.commitProduct = function(proId) {
						// 将productInfo的数据转换成我们的参数格式
						var str = "";
						for ( var x in $scope.productInfo) { // 迭代productInfo的所有属性
							if (x === "proEarningRate") { // 如果是这个属性它封装的是所有的利率信息，封装后的效果{12:1.2,16:1.6}
								var rateStr = "";
								for (var y = 0; y < $scope.productInfo.proEarningRate.length; y++) {
									rateStr += '"'
											+ $scope.productInfo.proEarningRate[y].month
								...
								str += "&" + x + "=" + $scope.productInfo[x]; // &
								// proId=1&proName=xx&proEarningRates={12:1.2,16:1.6}
							}
						}
						if (str.substring(0, 1) === '&') {
							str = str.substring(1);
						}
						alert(str); // proId=1&proName=xx&proEarningRates={12:1.2,16:1.6}
						// 和服务器发送请求
								...
								.success(
										function(response) {
											if (response.status == 1) {
												...
												// {proId:1,proEarningRate:[{month:12,incomeRate:1.2},{month:12,incomeRate:1.2}]}
		...
		.controller(
				...
				function($scope, $state, AuthService, PostService, hmd,
					...
					$scope.entering = function() {
						getDateVal(); // 所有数据已经绑定到params中
						if (formInvail()) { // 校验数据
							return;
						}
						// 下面是将绑定数据封装成我们向服务器提交时的格式
						var str = ""
						for ( var i in $scope.params) {
				...
				function($scope, $state, AuthService, PostService, hmd,
					...
					$scope.auditObj = {
						...
						submit : function() {
							...
							// var str = "ids=" + ids + "&audit_result="
							// + audit_result + "&audit_flg=" + audit_flg

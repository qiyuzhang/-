...
import cn.itcast.dao.product.MatchedModelRepository;
...
public class MatchService {
    ...
    public void startMatch() {
        ...
        List<FundingNotMatchedModel> fnmms = matchedModelRepository.findMatchFundingNotMatched();
        // 2. 得到所有待匹配的债权List<CreditorModel> 已审核11302 未匹配 11403 部分匹配 11401
        List<CreditorModel> cms = creditorModelRepository.findMatchCreditor();
    ...
    private void udateCreditor(List<CreditorModel> cms, LinkedList<CreditorModel> clist) {
        ...
        for (CreditorModel cm : clist) {
            ...
            if (_cm.getAvailableMoney() != cm.getAvailableMoney()) {
                ...
                creditorModelRepository.updateCreditorStatus(id, cm.getAvailableMoney(),
    ...
    private void updateCreditorStatus(List<CreditorModel> cms) {
        for (CreditorModel cm : cms) {
            creditorModelRepository.updateCreditorStatus(cm.getId(), 0, cm.getAvailableMoney(), 11402);
        }
    }
    ...
    private void updateInvestStatus(List<FundingNotMatchedModel> fnmms) {
        for (FundingNotMatchedModel fnmm : fnmms) {
            ...
            matchedModelRepository.updateStatus(fnmm.getfId());
        }
    }
